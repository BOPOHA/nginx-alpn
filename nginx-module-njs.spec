...
Requires: nginx == %{?epoch:%{epoch}:}1.15.6-1%{?dist}.ngx
...
%define MODULE_CONFIGURE_ARGS $(echo "--add-dynamic-module=njs-0.2.5/nginx")
...
%build
cd %{bdir}/njs-0.2.5 && ./configure && make njs
cd %{bdir}
...
%{__install} -m644 %{bdir}/njs-0.2.5/CHANGES $RPM_BUILD_ROOT%{_datadir}/doc/nginx-module-njs/
%{__mkdir} -p $RPM_BUILD_ROOT%{_bindir}
%{__install} -m755 %{bdir}/njs-0.2.5/build/njs $RPM_BUILD_ROOT%{_bindir}/
